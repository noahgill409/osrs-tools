...
def unidirectional_transition_matrix(P: np.ndarray) -> bool:
	...
	# conditions = []
	# conditions.append(m == n)
	# conditions.append(all([math.fabs(sum(P[i, :]) - 1) < precision for i in range(n)]))
	# conditions.append(all(all([P[i, j] == 0 for j in range(i)]) for i in range(n)))
	# return bool(math.prod(conditions))
